...
namespace BMCSDL.DAL
{
    class DALGiamSat
    {
        ...
        {
            DataTable table = new DataTable();
            OracleDataAdapter adapter = new OracleDataAdapter("select IDUSERXD,ID from TIN.PASSPORTDATA where TTXACTHUC='Đã xác thực' and TTXETDUYET!='Chưa duyệt'", mydb.getConnectionNV());
            adapter.Fill(table);
        ...
        {
            DataTable table = new DataTable();
            OracleDataAdapter adapter = new OracleDataAdapter("select IDUSERTT,ID from TIN.PASSPORTDATA where TTXETDUYET!='Chưa duyệt' and TTThongBao!='Chưa thông báo'", mydb.getConnectionNV());
            adapter.Fill(table);
